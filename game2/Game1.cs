...
namespace game2
{
    public class Game1 : Game
    {
        ...
        // Changing this value will now automatically resize all sprites in-game
        ...
        protected override void Initialize()
        {
            ...
            // Fix: Offset every waypoint in the path by half a tile size
            for (int i = 0; i < _mapGen.Path.Count; i++)
            {
                _mapGen.Path[i] = new Vector2(
                    _mapGen.Path[i].X + (TileSize / 2),
                    _mapGen.Path[i].Y + (TileSize / 2)
                );
            }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            HandleInput();
            _spawner.Update(gameTime, _currentWave, _mapGen.Path, _enemies);
        ...
        private void HandleInput()
        {
            ...
            if (currentMouse.LeftButton == ButtonState.Pressed && _previousMouseState.LeftButton == ButtonState.Released)
            {
                ...
                if (gX >= 0 && gX < _mapGen.GridMap.GetLength(0) && gY >= 0 && gY < _mapGen.GridMap.GetLength(1))
                {
                    ...
                    {
                        // Calculate the center of the tile dynamically
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            // PointClamp keeps your pixel art sharp when resized
            ...
            for (int x = 0; x < _mapGen.GridMap.GetLength(0); x++)
            {
                for (int y = 0; y < _mapGen.GridMap.GetLength(1); y++)
                {
                    ...
                    Color tileColor = _mapGen.GridMap[x, y] == 1 ? Color.Gray : Color.ForestGreen;
