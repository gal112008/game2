...
namespace game2
{
    public class MapGenerator
    {
        ...
        public void Generate()
        {
            ...
            {
                int action = rand.Next(0, 2); // 0: Move Forward, 1: Loop, 2: Random Turn
                ...
                {
                    LoopUpStart(ref currentX, ref currentY, 0, ref last);
                    currentX += 2;
                    Path.Add(new Vector2(currentX * _tileSize, currentY * _tileSize));
                   didyounotloop = 0;
                }
        ...
        public bool CanYouLoopThatWay(int currentX, int currentY, int iswhat)
        {
            switch (iswhat)
            {
                case 0: // Checking RIGHT
                    ...
                    return (currentX + 3 < _cols) && (currentY - 2 >= 0) && (currentY + 2 < _rows);

                case 3: // Checking LEFT
                        // Needs 2 tiles left, and 2 tiles both up and down
                    return (currentX - 3 >= 0) && (currentY - 2 >= 0) && (currentY + 2 < _rows);
        ...
        public void LoopUpStart(ref int currentX, ref int currentY, int iswhat,ref int last)
        {
            Random rand = new Random();
            int upordown = rand.Next(0, 2);

            if (last != 1 && CanYouGoThatWay(1, currentX, currentY - 3))
            {
                currentY -= 3;
                Path.Add(new Vector2(currentX * _tileSize, currentY * _tileSize));
                last = 1;
            ...
            else if (last != 0 && CanYouGoThatWay(2, currentX, currentY + 3))
            {
                currentY += 3;
                Path.Add(new Vector2(currentX * _tileSize, currentY * _tileSize));
                last = 0;
            }
            if (iswhat == 0) currentX += 2;//right
            else if (iswhat == 3) currentX -= 2;//left
            else if (iswhat == 1) currentY -= 2;//up
            else if (iswhat == 2) currentY += 2;//down
            Path.Add(new Vector2(currentX * _tileSize, currentY * _tileSize));
